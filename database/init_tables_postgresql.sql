...
-- ----------------------------
DROP TABLE IF EXISTS jiqizhixin_article CASCADE;
...
-- 创建唯一索引
CREATE UNIQUE INDEX idx_jiqizhixin_article_unique ON jiqizhixin_article(article_id);
CREATE INDEX idx_jiqizhixin_article_date ON jiqizhixin_article(publish_date);
CREATE INDEX idx_jiqizhixin_article_time ON jiqizhixin_article(publish_time);

-- 表注释
COMMENT ON TABLE jiqizhixin_article IS '机器之心文章表';
COMMENT ON COLUMN jiqizhixin_article.id IS '自增ID';
COMMENT ON COLUMN jiqizhixin_article.article_id IS '文章唯一ID';
COMMENT ON COLUMN jiqizhixin_article.title IS '文章标题';
COMMENT ON COLUMN jiqizhixin_article.description IS '文章描述/摘要';
COMMENT ON COLUMN jiqizhixin_article.content IS '文章内容';
COMMENT ON COLUMN jiqizhixin_article.article_url IS '文章链接';
COMMENT ON COLUMN jiqizhixin_article.author IS '文章作者';
COMMENT ON COLUMN jiqizhixin_article.publish_time IS '发布时间戳';
COMMENT ON COLUMN jiqizhixin_article.publish_date IS '发布日期(YYYY-MM-DD)';
COMMENT ON COLUMN jiqizhixin_article.read_count IS '阅读数';
COMMENT ON COLUMN jiqizhixin_article.like_count IS '点赞数';
COMMENT ON COLUMN jiqizhixin_article.comment_count IS '评论数';
COMMENT ON COLUMN jiqizhixin_article.share_count IS '分享数';
COMMENT ON COLUMN jiqizhixin_article.collect_count IS '收藏数';
COMMENT ON COLUMN jiqizhixin_article.category IS '文章分类';
COMMENT ON COLUMN jiqizhixin_article.tags IS '文章标签(JSON格式)';
COMMENT ON COLUMN jiqizhixin_article.cover_image IS '文章封面图片URL';
COMMENT ON COLUMN jiqizhixin_article.source_keyword IS '来源关键词';
COMMENT ON COLUMN jiqizhixin_article.is_original IS '是否原创(1-是, 0-否)';
COMMENT ON COLUMN jiqizhixin_article.reference_links IS '文章中的参考链接(JSON格式)';
COMMENT ON COLUMN jiqizhixin_article.add_ts IS '记录添加时间戳';
COMMENT ON COLUMN jiqizhixin_article.last_modify_ts IS '记录最后修改时间戳';
