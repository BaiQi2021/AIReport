...
# -*- coding: utf-8 -*-
"""
AI News Sites Scraper (机器之心、新智元等)
爬取AI新闻网站的资讯
"""
...
class XinzhiyuanScraper(BaseWebScraper):
    """新智元爬虫"""
    
    def __init__(self):
        super().__init__(
            base_url="https://www.xinzhiyuan.cn",  # 尝试 .cn 域名
            company_name="xinzhiyuan"
        )
    ...
    async def get_article_list(self, page: int = 1) -> List[Dict]:
        ...
        try:
            ...
            logger.info(f"Fetching Xinzhiyuan article list page {page}...")
            ...
            # 新智元的文章结构: 查找包含 href 的 article 或 div
            article_elements = soup.find_all(['article', 'div', 'li'], class_=lambda x: x and any(keyword in str(x).lower() for keyword in ['article', 'item', 'post', 'news', 'card']))
            
            if not article_elements:
                article_elements = soup.select('a[href*="/article/"], a[href*="/p/"]')
            ...
            for elem in article_elements[:30]:
                try:
                    ...
                    if not title_elem:
                        title_elem = link_elem
                        # 尝试从 title 属性获取
                        if not title_elem.get_text(strip=True):
                    ...
                    articles.append({
                        'article_id': f"xinzhiyuan_{article_id}",
                        'title': title[:500],
                        'url': url,
                    })
            ...
            logger.info(f"Page {page}: Extracted {len(articles)} Xinzhiyuan articles")
            return articles
        
        except Exception as e:
            logger.error(f"Failed to get Xinzhiyuan article list: {e}")
            return []
    
    async def get_article_detail(self, article_id: str, url: str) -> Optional[Dict]:
        """获取文章详情"""
        try:
            logger.info(f"Fetching Xinzhiyuan article details: {article_id}")
            ...
            if not title_elem:
                ...
                if title_elem:
                    title_text = title_elem.get_text(strip=True)
                    article['title'] = title_text.split('_')[0].strip()
                else:
                    article['title'] = ''
            else:
                article['title'] = self.clean_text(title_elem.get_text())
            ...
            # 作者
            author_elem = soup.find(class_=lambda x: x and 'author' in str(x).lower())
            article['author'] = self.clean_text(author_elem.get_text()) if author_elem else '新智元'
            ...
            logger.error(f"Failed to get Xinzhiyuan article details {article_id}: {e}")
            return None
...
async def run_xinzhiyuan_crawler(days: int = 7):
    """运行新智元爬虫"""
    logger.info("=" * 60)
    logger.info("🚀 Xinzhiyuan Crawler Started")
    logger.info("=" * 60)
    
    start_date = (datetime.now() - timedelta(days=days)).replace(hour=0, minute=0, second=0, microsecond=0)
    logger.info(f"Date range: {start_date.date()} to {datetime.now().date()}")
    
    scraper = XinzhiyuanScraper()
    await scraper.init()
    ...
    try:
        ...
        while page <= max_pages:
            ...
            for article_item in articles:
                try:
                    ...
                    logger.error(f"Error processing Xinzhiyuan article: {e}")
                    continue
        ...
        await scraper.close()
        logger.info("Xinzhiyuan Crawler finished.")


if __name__ == "__main__":
    async def main():
        await run_jiqizhixin_crawler()
        await run_xinzhiyuan_crawler()
